...
// 👇 تأكد من وجود ملف الأسئلة في نفس المجلد
const questionsDB = require('./questions.json'); 

// 1. تعريف التطبيق (app) أولاً
const app = express(); 
...
// 3. 👇👇👇 إنشاء السيرفر هنا (ده اللي كان ناقص ومكانه غلط) 👇👇👇
...
// دالة الحماية (تمنع السيرفر من الوقوع)
...
io.on('connection', (socket) => {
  ...
  // --- تعريف الدوال المساعدة داخل الاتصال (لتجنب مشاكل الوصول لـ io) ---
  ...
  const startGuessingPhase = (game) => {
    let options = [{ text: game.gameState.currentQuestion.a, type: 'REAL', owner: 'server' }]; 
  ...
  const startCategorySelectionPhase = (game) => {
    ...
    // التحقق من التعادل (Sudden Death)
